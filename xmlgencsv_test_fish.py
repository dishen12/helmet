...
def getAllXml(path):
    ...
    for root, dirs, files in os.walk(path):
        for img_name in files:
            ...
            if (os.path.isfile (xmlpath) == True):
...
if __name__ == '__main__':
    path="/home/sy/crab/unused/fish/"
    txt,cls=getAllXml(path)
    title="iscas_frcnn_test_fish"
    cnt = 1
    ...
    title = "iscas_frcnn_class_test_fish"
    id = 0
    with open ("%s.csv" % title, "w") as f:
        print('female_sturgeon,0',file = f)
        print('male_sturgeon,1',file = f)
        print('eel,2',file = f)
    f.close()
